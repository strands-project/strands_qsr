...
if __name__ == '__main__':
    ...
    for geo in geo_states:
        ...
        for ob1 in geo._objects.keys():
            for ob2 in geo._objects.keys():
                ...
                if not relatives.has_key(pair):
                    relatives[pair] = []
                relatives[pair].append( delta  + list(s1) + list(s2) )
    ...
    for pair_relatives in relatives.values():
        ##############################################################################
        ...
        #X = StandardScaler().fit_transform(np.array(relatives))
        X = np.array(pair_relatives)
        ...
        print labels
        for l, rel in zip(labels, pair_relatives):
            if l != -1:
                sets[int(l)].append(rel)
        for i, s in enumerate(sets):
            g = ds.GaussKernel(np.array(s), 1)
            q =  qualitators.SimpleGaussianQualitator("rel%d"%i, 2, g, std_threshold=0.3)
            qs.add_qualitator(q)
